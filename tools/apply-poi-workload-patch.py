...
new_function = r'''  async function loadPois(parsed, preserveSelection = false, retryFailedOnly = false) {
...
text, count = re.subn(
    r"  async function loadPois\(parsed, preserveSelection = false, retryFailedOnly = false\) \{.*?\n  \}\n\n  async function loadFile",
    new_function + "\n  async function loadFile",
    text,
    flags=re.S,
)
if count != 1:
    raise SystemExit(f'Expected one loadPois replacement, got {count}')
